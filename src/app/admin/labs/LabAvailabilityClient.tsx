...
import { updateLabAvailability } from "./actions";
...
export function LabAvailabilityClient({
  labId,
  bookingWindowStart,
  bookingWindowEnd,
  bookingDays,
  ...
  function handleSave(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await updateLabAvailability(labId, {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className={`text-xs font-bold px-2.5 py-1 rounded border transition-colors whitespace-nowrap ${
          hasWindow
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl p-4 w-72 space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-bold text-gray-900">Booking Window</h4>
            <button
              type="button"
