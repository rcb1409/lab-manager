...
export function EditEquipmentClient({ equipment }: { equipment: Equipment }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateEquipment(equipment.id, { name, type, description, imageUrl });
      setIsOpen(false);
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        `Delete "${equipment.name}"? This will permanently remove the equipment, its calendar, bookings, and usage history. This cannot be undone.`
      )
    )
      return;
    setDeleting(true);
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-2xl overflow-hidden max-w-md w-full max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-[#f2f2f2]">
              <h3 className="font-bold text-lg font-slab tracking-tight">Edit Equipment</h3>
